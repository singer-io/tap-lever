"""Opportunity stream with nested child-stream syncs for the Lever tap."""
...
class OpportunityStream(TimeRangeStream):
    """All opportunities in the Lever account, synced incrementally by updated_at."""
    ...
    def sync(self, child_streams=None):
        """Write the schema and sync opportunities together with their child streams."""
        LOGGER.info(
    ...
    def sync_paginated(self, url, params=None, updated_after=None, child_streams=None):
        """Paginate through opportunities and sync child records for each opportunity."""
        table = self.TABLE

        transformer = singer.Transformer(singer.UNIX_MILLISECONDS_INTEGER_DATETIME_PARSING)
        applications_stream = OpportunityApplicationsStream(
            self.config, self.state,
            child_streams.get('opportunity_applications'), self.client,
        )
        offers_stream = OpportunityOffersStream(
            self.config, self.state,
            child_streams.get('opportunity_offers'), self.client,
        )
        referrals_stream = OpportunityReferralsStream(
            self.config, self.state,
            child_streams.get('opportunity_referrals'), self.client,
        )
        resumes_stream = OpportunityResumesStream(
            self.config, self.state,
            child_streams.get('opportunity_resumes'), self.client,
        )
        ...
        while not finished_paginating:
            ...
            except OffsetInvalidException:
                LOGGER.warning(
                    'Found invalid offset "%s", retrying without offset.',
                    params['offset'],
                )
                params.pop("offset")
                _next = None
            ...
            if _next:
                params['offset'] = _next
                self.state = singer.bookmarks.write_bookmark(
                    self.state, table, "offset", _next,
                )
                self.state = singer.bookmarks.write_bookmark(
                    self.state, table, "next_page", page,
                )
                # Save the last_record bookmark when paginating to allow resume if interrupted
                self.state = singer.bookmarks.write_bookmark(
                    self.state, table, "last_record", updated_after.isoformat(),
                )
    ...
    def sync_data_for_period(self, date, interval, child_streams=None):
        """Sync opportunities in a single time window and advance the bookmark."""
        table = self.TABLE
        ...
        self.state = incorporate(
            self.state, table, self.RANGE_FIELD, date.isoformat(),
        )

        save_state(self.state)

    def sync_data(self, child_streams=None):
        """Sync all opportunity time windows from the last bookmark to now."""
        table = self.TABLE
